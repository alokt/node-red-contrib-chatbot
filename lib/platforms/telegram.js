...
Telegram.in(function(message) {
  return new Promise(function(resolve) {
    if (message.originalMessage.new_chat_title != null) {
      message.payload.content = message.originalMessage.new_chat_title;
      message.payload.type = 'event';
      message.payload.eventType = 'new_chat_title';
      resolve(message);
    } else {
      resolve(message);
    }
...
Telegram.out('message', function(message) {
  ...
  return new Promise(function (resolve, reject) {
    let task = null;
    if (modifyMessageId != null) {
      task = connector.editMessageText(content, {
      ...
      _(utils.split(content, MESSAGE_MAX_SIZE))
        .each(partial => {
          task = task.then(() =>
...
Telegram.registerEvent('new_chat_title', 'New chat title');
